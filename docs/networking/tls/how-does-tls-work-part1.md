...
private CAs and self-signed certificates and MITM all of their machines of the purposes of
controlling traffic on them[^7].
...
replace the self signed certificate on the uHTTPd web server on my router with
[OpenWRT](https://openwrt.org). For reference, this is how a self signed certificate provided by
OpenWRT looks like
